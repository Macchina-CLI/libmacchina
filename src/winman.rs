...
fn xprop_wm_detection() -> Result<String, ReadoutError> {
    if !extra::which("xprop") {
        return Err(ReadoutError::Other(String::from(
            "xprop command is not installed",
        )));
    }
    ...
    let window_manager_name_info =
        String::from_utf8(xprop_property_output.stdout).unwrap_or_else(|_| {
            panic!(
        ...
        .find(|line| line.starts_with("_NET_WM_NAME"))
    {
        ...
        Err(ReadoutError::Other(String::from(
            "Window manager detection with xprop failed",
        )))
    }
}
...
pub fn detect_wayland_window_manager() -> Result<String, ReadoutError> {
    if let Ok(wm) = xprop_wm_detection() {
        return Ok(wm);
    }
